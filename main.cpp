...
Microsoft::WRL::ComPtr<ID3D12Resource> CreateBufferResource(Microsoft::WRL::ComPtr<ID3D12Device> device, size_t sizeIntBytes) {
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	dsvDesc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D; // 2Texture
	// DSVheapの先頭にDSVを作る
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> wvpResource = CreateBufferResource(device, sizeof(TransfornatuinMatarix));
	// データを書き込む
	TransfornatuinMatarix* wvpDate = nullptr;
	// 書き込むためのアドレスを取得
	...
	const uint32_t kVertexCount = kSubdivision * kSubdivision * 6;
	 ...
	 // 頂点リソースを作る
	Microsoft::WRL::ComPtr<ID3D12Resource> vertexResource = CreateBufferResource(device, sizeof(VertexData) * kVertexCount);
	// 頂点バッファビューを作成する
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> transformationMatrixResourceSprite = CreateBufferResource(device, sizeof(TransfornatuinMatarix));
	// データを書き込む
	TransfornatuinMatarix* transformationMatrixDataSprite = nullptr;
	// 書き込むためのアドレスを取得
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			ImGui::ColorEdit4("*", &directionalLightData->color.x);
			...
			if (ImGui::CollapsingHeader("Sprite")) {
				ImGui::DragFloat3("scaleobj", &transformSprite.scale.x, 0.01f);
				ImGui::DragFloat3("rotateobj", &transformSprite.rotate.x, 0.01f);
				ImGui::DragFloat3("transformobj", &transformSprite.translate.x);
			}

			if (ImGui::CollapsingHeader("Sprite2")) {
				ImGui::Checkbox("useMonsteBall", &useMonsterBall);
